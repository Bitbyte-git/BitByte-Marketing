...
// ══════════════════════════════════════════════════════════════════
// ROLE CONFIG — promotor's own children: customer mattum (1 level)
// ══════════════════════════════════════════════════════════════════
const ROLE_CFG = {
...
function getPrintStyles(accent) {
  return `
    * { margin:0; padding:0; box-sizing:border-box; -webkit-print-color-adjust:exact; print-color-adjust:exact; }
    ...
    .header h1 { font-size:20px; font-weight:800; background:linear-gradient(90deg,#d4a017,#fb7185); -webkit-background-clip:text; background-clip:text; color:transparent; letter-spacing:0.3px; }
...
function showChainPopup(anchorEl, ancestors, current, dark) {
  ...
  if (!document.getElementById('chain-popup-pr-styles')) {
    ...
    s.textContent = `
      ...
      #chain-popup-pr::-webkit-scrollbar-thumb:hover{background:linear-gradient(180deg,#f0c419,#fda4af)}
      ...
      @keyframes acpGlow{0%,100%{box-shadow:0 0 0px rgba(212,160,23,0)}50%{box-shadow:0 0 20px rgba(212,160,23,0.22)}}
  ...
  const itemsHtml = chain.map((item, idx) => {
    ...
    const iconKey = item.type === 'promotor' ? 'star' : 'user'
  ...
  el.innerHTML = `
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:16px;padding-bottom:14px;border-bottom:1px solid ${isDark ? 'rgba(212,160,23,0.1)' : 'rgba(37,99,235,0.08)'};">
      <div style="display:flex;align-items:center;gap:9px;">
        ...
        <div>
          <div style="font-size:11px;color:${isDark ? '#f0c419' : '#16a34a'};font-weight:800;letter-spacing:1.8px;">HIERARCHY CHAIN</div>
...
// LANE CARD
// ══════════════════════════════════════════════════════════════════
function LaneCard({ node, role, ancestors, dark, text, subtext, onMessage }) {
  const navigate = useNavigate()
  const cfg = ROLE_CFG[role]
  const c = role === 'customer' ? STATUS_COLOR.green : cfg.color
  const Icon = cfg.Icon

  return (
    <div
      className="gcard gcard-active"
      style={{ '--nc': c }}
    ...
    >
      ...
      <div className="gcard-actions">
        <button
          onClick={e => { e.stopPropagation(); printPersonCard(node, role, cfg, c, ancestors) }}
...
// LANE ROW
// ══════════════════════════════════════════════════════════════════
function LaneRow({ role, items, ancestors, dark, text, subtext, emptyText, onMessage }) {
  const cfg = ROLE_CFG[role]
  return (
    <div className="glane">
...
// ══════════════════════════════════════════════════════════════════
// MAIN PAGE — Promotor login: shows ONLY this promotor's own customers
// ══════════════════════════════════════════════════════════════════
export default function Promotor_Hierarchy_grid() {
  const navigate = useNavigate()
  ...
  return (
    ...
    <div style={{ minHeight: '100vh', background: 'radial-gradient(circle at 20% 0%, #0b1a2e 0%, #020617 55%)', color: text, fontFamily: '"Inter",system-ui,sans-serif', padding: '28px 32px' }}>
      <style>{`
        ...
        .gcard{ background:linear-gradient(160deg, rgba(255,255,255,0.05), rgba(255,255,255,0.015)); border:1.5px solid var(--nc); border-radius:16px; padding:14px 18px; min-width:172px; max-width:210px; cursor:default; position:relative; transition:opacity .2s ease, transform .2s cubic-bezier(0.22,1,0.36,1), box-shadow .2s ease; flex-shrink:0; }
        ...
        .gcard-active{ opacity:1; box-shadow:0 0 0 1.5px var(--nc), 0 14px 30px rgba(0,0,0,0.45); }
  ...
  <button onClick={() => navigate('/promotor-hierarchy')} title="Switch to Tree View"
  ...
  <span style={{ color: '#a5f3fc', fontSize: '16px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
    My Team Hierarchy
      ...
      <div style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${border}`, borderRadius: '20px', padding: '24px 28px', minHeight: '70vh' }}>
        ...
        {!loading && root && debouncedSearch ? (
          ...
          <>
            {/* ── viewer's own promotor node ── */}
            <div className="gsa-card" style={{ borderColor: ROLE_CFG.promotor.color, background: `${ROLE_CFG.promotor.color}14` }}>
            ...
            <LaneRow role="customer" items={customers} ancestors={customerAncestors}
              dark={dark} text={text} subtext={subtext}
              emptyText="No customers under you yet." onMessage={openMessagePopup} />
          </>
        )}
      </div>
    ...
    {messageTarget && (
      ...
      >
        <div onClick={e => e.stopPropagation()} style={{ background: 'linear-gradient(145deg,#0a1628,#060e1c)', border: '1px solid rgba(212,160,23,0.3)', borderRadius: '20px', padding: '28px', width: '95%', maxWidth: '460px', boxShadow: '0 32px 80px rgba(0,0,0,0.7)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '18px' }}>
            <div>
              <div style={{ color: '#d4a017', fontWeight: 800, fontSize: '14px', letterSpacing: '0.05em', display: 'flex', alignItems: 'center', gap: '6px' }}>
                <IconMessage color="#d4a017" size={14} /> SEND DIRECT MESSAGE
              </div>
          ...
          <button
            ...
            style={{ marginTop: '16px', width: '100%', padding: '13px', background: messageSending ? 'rgba(212,160,23,0.25)' : 'linear-gradient(90deg,#d4a017,#fb7185)', border: 'none', borderRadius: '12px', fontWeight: 800, fontSize: '14px', color: messageSending ? '#d4a017' : '#3b0024', cursor: (messageSending || !messageTitle.trim() || !messageBody.trim()) ? 'not-allowed' : 'pointer', transition: 'all 0.3s ease', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '8px' }}
          >
            {messageSending ? (
              <>
                <div style={{ width: 14, height: 14, border: '2px solid rgba(212,160,23,0.3)', borderTop: '2px solid #d4a017', borderRadius: '50%', animation: 'spin 0.8s linear infinite' }} />
                Sending...
              </>
            ) : (
              <><IconMessage color={(messageSending || !messageTitle.trim() || !messageBody.trim()) ? '#d4a017' : '#3b0024'} size={14} /> Send to this person only</>
            )}
          </button>
        </div>
      </div>
