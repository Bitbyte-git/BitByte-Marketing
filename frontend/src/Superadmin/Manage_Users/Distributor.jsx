...
export default function Distributor() {
  ...
  const fetchData = async (signal, currentOffset, searchTerm, append, retryCount = 0) => {
    ...
    try {
      ...
      // NEW: server busy-ah irundhu fail aanaalum, "No data" kaamikkathu —
      // skeleton-ah vachukondே, 1.5s wait pannitu automatic retry pannuvom (max 5 tries)
      if (retryCount < 5) {
        setTimeout(() => {
          fetchData(signal, currentOffset, searchTerm, append, retryCount + 1)
        }, 1500)
      } else {
        // ella retry-um fail aana mattum loading நிறுத்தி, empty message காட்டு
        if (!append) setRows([])
        setLoading(false)
        setLoadingMore(false)
      }
      ...
      <div style={{ background: 'rgba(253,253,252,0.97)', border: `1px solid ${border}`, borderRadius: '22px', padding: '34px 38px', boxShadow: '0 22px 58px rgba(7,59,63,0.08)' }}>
        ...
        {loading ? (
          ...
          <p style={{ color: subtext, textAlign: 'center', padding: '60px 0', fontSize: '15px' }}>
            {search ? `No results for "${search}"` : 'No Distributors yet!'}
          </p>
        ) : (
          <div style={{ overflowX: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: '15px' }}>
              ...
              <tbody>
                {rows.map((a, i) => (
                  <tr key={a.id || a.dealer_id} style={{ borderBottom: '1px solid rgba(12,64,68,0.16)' }}>
                    ...
                    <td style={{ padding: '14px 16px', color: text, fontFamily: 'monospace', fontWeight: 800 }}>{a.dealer_id}</td>
                      ...
                      {actionOpen === a.id && (
                        <div className="ss-action-menu">
                          ...
                          <button type="button" onClick={async () => {
                            await navigator.clipboard.writeText(a.dealer_id || '')
                            setCopiedId(a.id)
        ...
        <div onClick={() => setSelectedDetail(null)} style={{ position: 'fixed', inset: 0, zIndex: 1350, padding: 20, display: 'grid', placeItems: 'center', background: 'rgba(7,31,34,.52)', backdropFilter: 'blur(8px)' }}>
          <section onClick={e => e.stopPropagation()} style={{ width: 'min(520px,100%)', overflow: 'hidden', border: '1px solid rgba(204,168,129,.38)', borderRadius: 22, background: 'linear-gradient(155deg,#fff,#F7FAF8)', boxShadow: '0 35px 90px rgba(7,31,34,.3)' }}>
            <header style={{ padding: '24px 26px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', color: '#fff', background: 'linear-gradient(120deg,#073B3F,#0C5254)' }}>
              <div>
                <small style={{ display: 'block', marginBottom: 5, color: '#D9B780', fontSize: 9, fontWeight: 800, letterSpacing: '.16em' }}>DISTRIBUTOR PROFILE</small>
            ...
            <div style={{ padding: 24, display: 'grid', gridTemplateColumns: 'repeat(2,minmax(0,1fr))', gap: 12 }}>
              {[['ID', selectedDetail.dealer_id], ['Email', selectedDetail.email], ['Mobile', selectedDetail.mobile_number], ['City', selectedDetail.city_name]].map(([label, value]) => (
            ...
            <footer style={{ padding: '0 24px 24px', display: 'flex', gap: 10 }}>
              <button type="button" onClick={() => navigate(`/hierarchy-sales-count?role=dealer&id=${selectedDetail.id}`)} style={{ flex: 1, minHeight: 44, border: 0, borderRadius: 11, color: '#fff', background: '#073B3F', fontWeight: 800, cursor: 'pointer' }}>Open performance</button>
