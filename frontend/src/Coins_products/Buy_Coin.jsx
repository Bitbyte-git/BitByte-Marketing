...
export default function BuyCoin() {
  ...
  const submitRequest = async () => {
    ...
    try {
      if (role === 'super_admin') {
        // Super Admin — direct-ah own stock ku add, approval venaam
        await api.post('/coin-stock/add/', { items: cart })
        setMsgType('success')
        setMsg('Coins added to your stock!')
      } else {
        ...
        <section className="bc-grid">
          ...
          <aside className="bc-cart">
            ...
            <div className="bc-summary">
              ...
              <div className="bc-row"><span>{role === 'super_admin' ? 'Destination' : 'Approver'}</span><b>{role === 'super_admin' ? 'Your Stock (Direct)' : (ROLE_TARGET[role] || 'Upstream')}</b></div>
            </div>
            ...
            <button type="button" className="bc-primary" disabled={submitting || !cart.length} onClick={submitRequest} style={{ width: '100%', opacity: submitting || !cart.length ? .58 : 1 }}>
              {submitting ? (role === 'super_admin' ? 'Adding...' : 'Sending...') : (role === 'super_admin' ? 'Add to Stock' : 'Send Coin Request')}
            </button>
          </aside>
        </section>
      </div>
